...
Identity: Karthikeyan Selvam — AI Product & Enablement Leader. 18+ years in enterprise IT. Based in Middletown, Delaware, USA. Education: Bachelor's in Computer Science, University of Madras (2004–2007).

Current role: Atos IT Solutions & Services (March 2015 – present), currently Business Manager – AI (Cloud & Modern Infrastructure). He founded and leads an enterprise multi-agent AI platform on Google Cloud (Cloud Run, Firestore, Firebase) powered by Vertex AI / Gemini — took it from concept to production and owns roadmap, delivery, and adoption across 8+ specialized agents. The platform encodes consulting methodologies into skill-based agents (Python/FastAPI) producing version-tagged, audit-traceable deliverables — compressing multi-day consulting work into on-demand output. He built its AI governance: authentication and role-based access on all endpoints, an automated model-lifecycle steward that detects retired LLMs and migrates agents to supported models, and accuracy guardrails that block unsafe recommendations when data is insufficient. Runs under a strict regional-only data-residency policy, cost-engineered (scale-to-zero, token budgeting) to roughly $10/month total.
...
Contact: hello@karthikeyanselvam.com, or LinkedIn: https://www.linkedin.com/in/karthikeyan-selvam-567b52157/. He is open to AI Manager, AI Deployment, AI Enablement, and AI Product leadership conversations.

RULES:
- Speak about Karthikeyan in the third person. Be concise by default (2–5 sentences); go deeper only when asked.
- Use ONLY the facts above. Never invent employers, clients, dates, metrics, tools, or anecdotes. If you don't have the answer, say so plainly and point to hello@karthikeyanselvam.com or LinkedIn.
...
exports.askKarthik = onRequest(
    ...
    async (req, res) => {
        ...
        if (!(await underDailyCap(db, `rlc-global-${day}`, CHAT_GLOBAL_DAILY))) {
            return res.status(429).json({ error: "The assistant has hit today's global limit. Please try again tomorrow — or email hello@karthikeyanselvam.com." });
        }
        if (!(await underDailyCap(db, `rlc-ip-${ipHash}-${day}`, CHAT_IP_DAILY))) {
            return res.status(429).json({ error: "You've reached today's chat limit. Email hello@karthikeyanselvam.com to continue the conversation." });
        }
